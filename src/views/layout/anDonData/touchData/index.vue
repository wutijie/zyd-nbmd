<template>
    <div class="anDonData-touchData">
		...
		<el-table :data="deviceInfo" style="width: 100%" height="calc(100vh - 240px)" id="out-table">
			<el-table-column fixed prop="devicesno" label="设备序列号" align="center"></el-table-column>
			<el-table-column prop="deviceid" label="时间" align="center"></el-table-column>
			<el-table-column prop="devicename" label="设备编号"  align="center"></el-table-column>
			<el-table-column prop="appearancesize" label="设备名称" align="center"></el-table-column>
			<el-table-column prop="param1" label="触屏内容" align="center"></el-table-column>
			<!-- <el-table-column
			fixed="right"
			label="操作"
			width="100">
...
<script>
	import { TouchpadData } from '../../../../api/deviceMation.js';
    ...
    export default {
		...
		created() {
			this.getNowDate();
			TouchpadData('','',this.timedate).then(res => {
				let that = this;
				if(res.code == 200) {
					console.log(res);
					that.devicesnoSelect = res.data.DeviceSelect;
					that.deviceInfo = res.data.TouchpadData;
		...
		methods:{
			...
			outExcel () {
				exportExcel("#out-table","安灯数据-触屏数据")
			},
			searchNumber(){
				let items = this.value.split("--");
				let custid = items[1];
				let deid = items[0];
				TouchpadData(deid,custid,this.timedate).then(res => {
					let that = this;
					if(res.code == 200) {
						console.log(res);
						that.deviceInfo = res.data.TouchpadData;
